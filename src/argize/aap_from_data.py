...
SIMPLE_TYPES: t.Mapping[t.Type[T], t.Callable[[str], T]] = {
    str: str,
    int: int,
    float: float,
...
class AapFromData(t.Generic[T]):
    ...
    def handle_explicit_type(self) -> None | argize.AddArgumentParameters[T]:
        if self.extra_info.add_argument_parameters.type is argize.NOT_SET:
            return None
        return self.extra_info.add_argument_parameters

    def handle_mapping(self) -> None | argize.AddArgumentParameters[T]:
        if self.extra_info.mapping is argize.NOT_SET:
            return None
        return argize.AddArgumentParameters(
                choices=list(self.extra_info.mapping.keys()),
                type=lambda x: self.extra_info.mapping[x],
        )
